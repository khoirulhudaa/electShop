    ...
    <>
       {
            products?.length !== 0 ? (
                products?.slice(0, 10).map((product, index) => (
                    <div key={index} className='w-[180px] mr-5 cursor-pointer border border-slate-300 rounded-lg h-[320px] bg-white rounded-lg overflow-hidden shadow-lg mr-20px'>
                        ...
                        <div className='w-full h-[45%] flex overflow-hidden flex-col p-3'>
                            <h3 className='text-[16px] w-max mb-2 overflow-hidden max-w-full whitespace-nowrap overflow-ellipsis'>{product?.product_name}</h3>
                            <div className='w-full items-center justify-between flex h-max flex-col'>
                                <div className='w-full flex items-center justify-between'>
                                    ...
                                    <div className='w-max flex items-center'>
                                        <div onClick={() => handleCart(generateRandomCharacter(5), product, dispatch)} className='w-[30px] hover:scale-[1.2] duration-100 text-white rounded-full bg-blue-500 h-[30px] text-[12px] flex items-center justify-center'>
                                            <FaCartPlus />
                                        </div>
                                    </div>
                                </div>
                ...
                <div className='w-[190px] mr-5 flex justify-center items-center cursor-pointer border border-slate-300 rounded-lg h-[320px] bg-white rounded-lg overflow-hidden shadow-lg mr-20px'>
